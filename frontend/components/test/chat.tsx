...
export default function ChatComponent({ channel = 'general', className = '' }: ChatComponentProps) {
    const { messages, sendMessage, isConnected, isSending } = useChat(channel);
    const { user } = useAuth() || {};
    ...
    return (
        <div className={`flex flex-col h-full bg-white rounded-lg shadow ${className}`}>
            ...
            <div className="flex items-center justify-between p-4 border-b">
                <div>
                    <h2 className="text-lg font-semibold">Chat: {channel}</h2>
