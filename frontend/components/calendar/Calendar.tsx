...
const Calendar: React.FC<CalendarProps> = ({
  ...
  const [hoveredTask, setHoveredTask] = useState<{ task: Task; position: { x: number; y: number } } | null>(null);
  ...
  const handleMouseEnter = (e: React.MouseEvent<HTMLDivElement>, task: Task) => {
  ...
  return (
    <>
      <div className={`rounded-lg p-2 relative ${className}`} style={{ isolation: 'isolate' }}>
        ...
        {nearestTask && (
          <div className="mb-4 p-3 bg-gradient-to-r from-orange-50 to-amber-50 rounded-lg border border-orange-200 relative z-0">
        ...
        <div className="p-3 bg-gray-50 rounded-lg mb-4 relative z-0">
        ...
        <div className="grid grid-cols-7 gap-1">
          {calendarDays.map((day, index) => (
            <div
              key={index}
              className="relative"
              onMouseEnter={(e) => day.tasks.length > 0 && handleMouseEnter(e, day.tasks[0])}
        ...
        {selectedDateTasks.length > 0 && (
          <div className="mt-4 pt-3 border-t border-gray-200 relative z-0">
        ...
        <div className="mt-4 pt-2 border-t border-gray-200 relative z-0">
      ...
      {/* Tooltip вынесен за пределы календаря */}
      {hoveredTask && (
        <div 
          className="fixed z-[9999] px-3 py-2 bg-gray-900 text-white text-xs rounded-lg shadow-lg pointer-events-none"
          style={{
            top: hoveredTask.position.y - 10,
            left: hoveredTask.position.x,
            transform: 'translate(-50%, -100%)',
            maxWidth: '200px'
          }}
        >
          <p className="font-medium truncate">{hoveredTask.task.title}</p>
          <p className="text-gray-300 text-xs mt-1">
            {format(new Date(hoveredTask.task.deadline), 'HH:mm')}
          </p>
          <div 
            className="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-1/2 rotate-45 w-2 h-2 bg-gray-900"
            style={{ bottom: '-4px' }}
          ></div>
        </div>
      )}
